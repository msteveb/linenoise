/*
 * Test framework imported from https://github.com/antirez/linenoise.git with modifications
 */
...
static int opt_render = 1;

#define EXEC_UNDER_TEST "./linenoise-example"

/* ========================= VT100 Emulator ========================= */

#define EMU_ROWS 15
#define EMU_COLS 60
#define EMU_COLS_STR "60"  /* For environment variable. */
...
static void emu_handle_csi(char cmd) {
    ...
    if (cmd != 'K' && n == 0) {
        /* For most commands, 0 is not a valid param and should be treated as 1 (default) */
        n = 1;
    }
...
#define MAX_DEBUG_BYTES 200
    if (emu_debug) {
        printf("EMU_FEED (%d bytes): ", len);
        for (int i = 0; i < len && i < MAX_DEBUG_BYTES; i++) {
            unsigned char c = buf[i];
            if (c >= 32 && c < 127) printf("%c", c);
            else printf("<%02X>", c);
        }
        if (len > MAX_DEBUG_BYTES) printf("...");
        printf("\n");
    }
    for (int i = 0; i < len; i++) {
...
static int test_start(const char *test_name, const char *program) {
    ...
    if (child_pid == 0) {
        ...
        setenv("LINENOISE_COLS", EMU_COLS_STR, 1);
...
static void test_end(void) {
    if (child_pid > 0) {
        /* Send CR, Ctrl-D (EOF) to terminate cleanly. */
        if (write(pipe_to_child[1], "\r\x04", 2) < 0) {
        ...
        if (wait_result == 0) {
            printf("Killing child process (PID %d) - did not exit after EOF\n", child_pid);
            /* Child didn't exit, send SIGTERM. */
            kill(child_pid, SIGTERM);
            /* Wait forever for the child to exit */
...
static void send_keys(const char *keys) {
    if (write(pipe_to_child[1], keys, strlen(keys)) < 0) {
        perror("write to child");
    }
...
static void assert_screen_row_(const char *file, int line, int row, const char *expected) {
    tests_run++;
    ...
    if (strcmp(actual, expected) == 0) {
        ...
        tests_failed++;
        printf("\x1b[31m[FAIL]\x1b[0m %s:%d Row %d:\n", file, line, row);
        printf("       Expected: \"%s\"\n", expected);
        printf("       Actual:   \"%s\"\n", actual);
    }
    fflush(stdout);
}
#define assert_screen_row(row, expected) assert_screen_row_(__FILE__, __LINE__, row, expected)

static void assert_cursor_(const char * file, int line, int row, int col) {
    tests_run++;
    if (emu_cursor_row == row && emu_cursor_col == col) {
        ...
        tests_failed++;
        printf("\x1b[31m[FAIL]\x1b[0m %s:%d Cursor position:\n", file, line);
        printf("       Expected: (%d, %d)\n", row, col);
        printf("       Actual:   (%d, %d)\n", emu_cursor_row, emu_cursor_col);
    }
    fflush(stdout);
...
#define assert_cursor(row, col) assert_cursor_(__FILE__, __LINE__, row, col)

static void assert_row_contains_(const char * file, int line, int row, const char *substr) {
    tests_run++;
    const char *actual = emu_get_row(row);
    if (strstr(actual, substr) != NULL) {
        ...
        tests_failed++;
        printf("\x1b[31m[FAIL]\x1b[0m %s:%d Row %d doesn't contain \"%s\"\n", file, line, row, substr);
        printf("       Actual: \"%s\"\n", actual);
    }
    fflush(stdout);
}

#define assert_row_contains(row, substr) assert_row_contains_(__FILE__, __LINE__, row, substr)
...
static void test_simple_typing(void) {
    if (test_start("Simple Typing", EXEC_UNDER_TEST) == -1) return;
...
static void test_cursor_movement(void) {
    if (test_start("Cursor Movement", EXEC_UNDER_TEST) == -1) return;
...
static void test_backspace_delete(void) {
    if (test_start("Backspace and Delete", EXEC_UNDER_TEST) == -1) return;
...
static void test_utf8_typing(void) {
    if (test_start("UTF-8 Typing", EXEC_UNDER_TEST) == -1) return;
...
static void test_utf8_emoji(void) {
    if (test_start("UTF-8 Emoji", EXEC_UNDER_TEST) == -1) return;
...
static void test_utf8_cursor_over_emoji(void) {
    if (test_start("UTF-8 Cursor Over Emoji", EXEC_UNDER_TEST) == -1) return;
...
static void test_utf8_backspace_emoji(void) {
    if (test_start("UTF-8 Backspace Emoji", EXEC_UNDER_TEST) == -1) return;
...
static void test_utf8_backspace_4byte_only(void) {
    if (test_start("UTF-8 Backspace 4-byte Only", EXEC_UNDER_TEST) == -1) return;
...
static void test_utf8_grapheme_clusters(void) {
    if (test_start("UTF-8 Grapheme Clusters", EXEC_UNDER_TEST) == -1) return;
...
static void test_utf8_grapheme_cursor_movement(void) {
    if (test_start("UTF-8 Grapheme Cursor Movement", EXEC_UNDER_TEST) == -1) return;
...
static void test_emulator_grapheme_storage(void) {
    if (test_start("Emulator Grapheme Storage", EXEC_UNDER_TEST) == -1) return;
...
static void test_ctrl_w_delete_word(void) {
    if (test_start("Ctrl-W Delete Word", EXEC_UNDER_TEST) == -1) return;

    send_keys("two words");
    assert_screen_row(0, "hello> two words");
    send_keys(KEY_CTRL_W);  /* Delete "world" */
    assert_screen_row(0, "hello> two");
...
static void test_ctrl_u_delete_line(void) {
    if (test_start("Ctrl-U Delete Line", EXEC_UNDER_TEST) == -1) return;
...
static void test_horizontal_scroll(void) {
    if (test_start("Horizontal Scroll", EXEC_UNDER_TEST) == -1) return;
    ...
    assert_cursor(0, EMU_COLS - 1);  /* At last column (60-col terminal) */
...
static void test_horizontal_scroll_utf8(void) {
    if (test_start("Horizontal Scroll UTF-8", EXEC_UNDER_TEST) == -1) return;
...
static void test_multiline_wrap(void) {
    if (test_start("Multiline Wrap", EXEC_UNDER_TEST " --multiline") == -1) return;
...
static void test_multiline_cursor_movement(void) {
    if (test_start("Multiline Cursor Movement", EXEC_UNDER_TEST " --multiline") == -1) return;
...
static void test_multiline_utf8(void) {
    if (test_start("Multiline UTF-8", EXEC_UNDER_TEST " --multiline") == -1) return;
...
static void test_multiline_history(void) {
    if (test_start("Multiline History Navigation", EXEC_UNDER_TEST " --multiline") == -1) return;
    ...
    send_keys("short");
    assert_row_contains(3, "hello> short");
    ...
    /* Verify row 6 is empty (no leftover from the long line). */
    assert_screen_row(6, "");
...
static void test_tab_no_completions(void) {
    if (test_start("TAB With No Completions", EXEC_UNDER_TEST) == -1) return;
...
#define ANSI_PROMPT "'\x1b[31mred\x1b[32mgreen\x1b[0m> '"
    if (test_start("ANSI Prompt Zero Width", EXEC_UNDER_TEST " --prompt " ANSI_PROMPT) == -1) return;
